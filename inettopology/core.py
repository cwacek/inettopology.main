...
from inettopology.util import uniqify, confirm
...
def parse_routes(args):
  ...
  for line in fin:
    ...
    as_path = uniqify(as_path_list, lambda x: x.strip("{}"),
    ...
    attrs = list(itertools.repeat({'source': os.path.basename(args.ribfile)},
                                  len(as_path)))
...
if __name__ == "__main__":
  ...
  read_parser.add_argument("--include-indirect",
  ...
  #Routeviews Parser
  route_parser = subp.add_parser("load_ribs",
                                 help="Add data from RIBs",
                                 parents=[gen_p])
  route_parser.add_argument("ribfile",
                            help="The RIB file in text format "
                                 "as output by bgpdump -M")
  route_parser.add_argument("tag",
                            help="The string to tag these entries "
                                 "with (probably a timestamp)")
